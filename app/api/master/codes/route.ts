...
export async function GET() {
  return createMasterCodeRouteHandlers(createQualityRepository(getDb(), schema)).GET();
}

export async function POST(request: Request) {
  return createMasterCodeRouteHandlers(createQualityRepository(getDb(), schema)).POST(request);
}
