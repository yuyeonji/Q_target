...
type MasterCode = { id: string; code: string; name: string; category: string; active: boolean };
...
const alarms: Alarm[] = [
  { id: "AL-99198", time: "2023-10-12 11:08:44", item: "Bearing Housing A1", type: "Sample Delay", process: "Machining", line: "Line 4", status: "심각", reviewer: "품질 검토팀" },
...
const initialAlarmRules: Rule[] = [
  { id: "ALR-001", name: "CPK 하한 경고", scope: "전 공장 / 가공", threshold: "1.33 미만", active: true },
  { id: "ALR-002", name: "불량률 급증", scope: "조립 2라인", threshold: "3.0% 초과", active: true },
  { id: "ALR-003", name: "샘플링 지연", scope: "전체 제품", threshold: "30분 초과", active: false },
];
const initialConversionRules: Rule[] = [
  { id: "CVR-001", name: "심각 알람 자동 전환", scope: "심각 등급 알람", threshold: "즉시 전환", active: true },
  { id: "CVR-002", name: "반복 알람 전환", scope: "동일 제품 / 동일 공정", threshold: "7일 내 3회", active: true },
  { id: "CVR-003", name: "장기 미검토 전환", scope: "신규·검토중 알람", threshold: "24시간 경과", active: false },
];
const initialCodes: MasterCode[] = [
  { id: "COD-001", code: "PRC-MCH", name: "가공", category: "공정 코드", active: true },
  { id: "COD-002", code: "ALM-CPK", name: "CPK 하락", category: "알람 유형", active: true },
  { id: "COD-003", code: "STS-HOLD", name: "출하 보류", category: "상태 코드", active: false },
];
...
export default function Home() {
  ...
  const [conversionRules, setConversionRules] = useState(initialConversionRules);
    ...
    <main className={compact ? "platform compact-mode" : "platform"}>
      ...
      <section className="workspace" inert={analysisPanel ? true : undefined}>
        ...
        <div className="content">
          ...
          {view === "master" && <Master tab={masterTab} setTab={setMasterTab} alarmRules={alarmRules} setAlarmRules={setAlarmRules} conversionRules={conversionRules} setConversionRules={setConversionRules} codes={codes} setCodes={setCodes} showNotice={showNotice} />}
        </div>
      </section>
      ...
      {analysisPanel && <AnalysisPanel panel={analysisPanel} onClose={closeAnalysis} />}
...
function Dashboard({ period, factory, product, setPeriod, setFactory, setProduct, onOpenAnalysis, onViewCriticalAlarms, onNavigate }: { period: string; factory: string; product: string; setPeriod: (value: string) => void; setFactory: (value: string) => void; setProduct: (value: string) => void; onOpenAnalysis: (panel: Exclude<AnalysisPanel, null>) => void; onViewCriticalAlarms: () => void; onNavigate: (view: ViewId, filter: string) => void }) {
  return <div className="dashboard-view">
    ...
    <div className="dashboard-grid"><article className="card chart-card"><h3>카테고리별 알람 추세 <button type="button" aria-label="카테고리별 알람 추세 상세 보기" onClick={() => onOpenAnalysis("trend")}>⋮</button></h3>
      {/* A focusable scroll region lets keyboard users reach bars that extend past a narrow viewport. */}
      {/* eslint-disable-next-line jsx-a11y/no-noninteractive-tabindex */}
      <div className="chart-scroll" role="region" aria-label="카테고리별 알람 추세 차트" tabIndex={0}><div className="chart"><span style={{height:"40%"}} /><span style={{height:"65%"}} /><span className="pink" style={{height:"84%"}} /><span style={{height:"45%"}} /><span style={{height:"30%"}} /><span style={{height:"55%"}} /><span style={{height:"70%"}} /><span style={{height:"90%"}} /><span style={{height:"60%"}} /><span style={{height:"40%"}} /></div></div><p className="legend">● 샘플 지연 · ● 공정능력 · ● 트렌드 이탈</p></article>
      <article className="card donut-card"><h3>대상 분포 <button type="button" aria-label="대상 분포 상세 보기" onClick={() => onOpenAnalysis("distribution")}>⋮</button></h3><div className="distribution-layout"><div className="donut"><strong>84<small>전체</small></strong></div><div className="distribution distribution-legend"><p><span className="distribution-dot normal" aria-hidden="true">●</span> 정상 <b>50 (60%)</b></p><p><span className="distribution-dot risk" aria-hidden="true">●</span> 위험 <b>22 (25%)</b></p><p className="red-text"><span className="distribution-dot overdue" aria-hidden="true">●</span> 기한 초과 <b>12 (15%)</b></p></div></div></article>
      <article className="card table-card"><h3>심각 알람 검토 대기 <button onClick={onViewCriticalAlarms}>전체 보기 →</button></h3><ScrollTable label="심각 알람 검토 대기 목록"><table><thead><tr><th>알람 ID</th><th>유형</th><th>공장 / 라인</th><th>지속 시간</th><th>상태</th></tr></thead><tbody>{[["ALM-8492","샘플 지연","알파 - L1","4h 12m"],["ALM-8490","공정능력 위반","베타 - L3","2h 45m"],["ALM-8488","트렌드 경고","감마 - L2","1h 10m"],["ALM-8475","샘플 지연","알파 - L1","5h 30m"]].map((r) => <tr key={r[0]}><td>{r[0]}</td><td>{r[1]}</td><td>{r[2]}</td><td>{r[3]}</td><td><Status>심각</Status></td></tr>)}</tbody></table></ScrollTable></article>
...
function AnalysisPanel({ panel, onClose }: { panel: Exclude<AnalysisPanel, null>; onClose: () => void }) {
  ...
  return <div className="overlay" onClick={(event) => { if (event.currentTarget === event.target) onClose(); }}>
    <aside ref={dialogRef} className="drawer" role="dialog" aria-modal="true" aria-labelledby="analysis-panel-title" tabIndex={-1}>
      ...
      {panel === "trend" ? <section>
        <h3>기간별 카테고리 추이</h3>
        <ScrollTable label="기간별 카테고리 추이 표"><table><thead><tr><th>카테고리</th><th>최근 7일</th><th>최근 30일</th><th>이번 분기</th></tr></thead><tbody>
        ...
        <h3>상태별 대상 분포</h3>
        <ScrollTable label="상태별 대상 분포 표"><table><thead><tr><th>상태</th><th>건수</th><th>비중</th><th>대상 행</th></tr></thead><tbody>
...
function Master({ tab, setTab, alarmRules, setAlarmRules, conversionRules, setConversionRules, codes, setCodes, showNotice }: { tab: string; setTab: (value: string) => void; alarmRules: Rule[]; setAlarmRules: (rules: Rule[]) => void; conversionRules: Rule[]; setConversionRules: (rules: Rule[]) => void; codes: MasterCode[]; setCodes: (codes: MasterCode[]) => void; showNotice: (message: string) => void }) {
  const tabs = ["알람 규칙", "전환 규칙", "코드 관리"];
  const description = tab === "알람 규칙" ? "알람 발생 기준과 감시 범위를 관리합니다." : tab === "전환 규칙" ? "알람을 관리대상으로 전환하는 조건을 관리합니다." : "제품·공정·상태 코드 체계를 관리합니다.";
  return <>
    <div className="page-head"><div><h2>마스터 관리</h2><p>시스템 핵심 규칙 및 기준정보를 설정합니다.</p></div><code>⌁ v4.2.1-prod</code></div>
    <div className="tabs" role="tablist" aria-label="마스터 관리 메뉴">{tabs.map((item) => <button type="button" role="tab" aria-selected={tab === item} key={item} onClick={() => setTab(item)} className={tab === item ? "selected" : ""}>{item}</button>)}</div>
    <p className="tab-description">{description}</p>
    <div className="master-grid">
      {tab === "알람 규칙" && <RuleManagement key="alarm" title="알람 규칙 관리" idLabel="알람 규칙 ID" scopeLabel="감시 범위" thresholdLabel="알람 임계값" idPrefix="ALR" filename="q-target-rules.csv" rules={alarmRules} setRules={setAlarmRules} showNotice={showNotice} />}
      {tab === "전환 규칙" && <RuleManagement key="conversion" title="전환 규칙 관리" idLabel="전환 규칙 ID" scopeLabel="전환 대상 범위" thresholdLabel="전환 임계값" idPrefix="CVR" filename="q-target-conversion-rules.csv" rules={conversionRules} setRules={setConversionRules} showNotice={showNotice} />}
      {tab === "코드 관리" && <CodeManagement codes={codes} setCodes={setCodes} showNotice={showNotice} />}
      <MasterNote codeMode={tab === "코드 관리"} />
    </div>
  </>;
}

function downloadMasterCsv(columns: string[], rows: string[][], filename: string) {
...
function RuleStateChoices({ active, label, onChange }: { active: boolean; label: string; onChange: (active: boolean) => void }) {
  return <div className="rule-state-choices" role="group" aria-label={label}>
    <button type="button" className={`rule-state-choice active ${active ? "selected" : ""}`} aria-pressed={active} onClick={() => onChange(true)}>활성</button>
    <button type="button" className={`rule-state-choice inactive ${!active ? "selected" : ""}`} aria-pressed={!active} onClick={() => onChange(false)}>비활성</button>
  </div>;
}

function CodeManagement({ codes, setCodes, showNotice }: { codes: MasterCode[]; setCodes: (codes: MasterCode[]) => void; showNotice: (message: string) => void }) {
  ...
  const addCode = (event: React.FormEvent) => {
    event.preventDefault();
    if (!draft.code.trim() || !draft.name.trim() || !draft.category.trim()) return showNotice("코드값, 코드명, 코드 분류를 모두 입력해 주세요.");
    const nextNumber = Math.max(0, ...codes.map((code) => Number(code.id.split("-")[1]) || 0)) + 1;
    setCodes([...codes, { id: `COD-${String(nextNumber).padStart(3, "0")}`, code: draft.code.trim().toUpperCase(), name: draft.name.trim(), category: draft.category.trim(), active: true }]);
    setDraft({ code: "", name: "", category: "" });
    showNotice("새 코드를 추가했습니다.");
  };
  const saveEdit = (event: React.FormEvent) => {
    event.preventDefault();
    if (!editing || !editing.code.trim() || !editing.name.trim() || !editing.category.trim()) return showNotice("코드값, 코드명, 코드 분류를 모두 입력해 주세요.");
    setCodes(codes.map((code) => code.id === editing.id ? { ...editing, code: editing.code.trim().toUpperCase(), name: editing.name.trim(), category: editing.category.trim() } : code));
    setEditing(null);
    showNotice("코드 변경사항을 저장했습니다.");
  };
  return <article className="card rules master-surface">
    <div className="section-head"><div><h3>코드 관리</h3><p>{codes.length}개 기준 코드</p></div><button type="button" onClick={() => { downloadMasterCsv(["코드 ID", "코드값", "코드명", "코드 분류", "상태"], codes.map((code) => [code.id, code.code, code.name, code.category, code.active ? "활성" : "비활성"]), "q-target-codes.csv"); showNotice("q-target-codes.csv 파일을 내려받았습니다."); }}>⇩ 내보내기</button></div>
    <form className="master-create code-create" onSubmit={addCode}>
      <label>코드값<input aria-label="새 코드값" value={draft.code} onChange={(event) => updateDraft("code", event.target.value)} placeholder="예: PRC-MCH" /></label>
      <label>코드명<input aria-label="새 코드명" value={draft.name} onChange={(event) => updateDraft("name", event.target.value)} placeholder="코드명 입력" /></label>
      <label>코드 분류<input aria-label="새 코드 분류" value={draft.category} onChange={(event) => updateDraft("category", event.target.value)} placeholder="분류 입력" /></label>
      <button className="black" type="submit">＋ 새 코드 추가</button>
    </form>
    <ScrollTable label="코드 관리 목록"><table><thead><tr><th>코드 ID</th><th>코드값</th><th>코드명</th><th>코드 분류</th><th>상태</th><th>작업</th></tr></thead><tbody>{codes.map((code) => editing?.id === code.id ? <tr className="master-edit-row" key={code.id}><td>{code.id}</td><td><input aria-label="코드값 수정" value={editing.code} onChange={(event) => setEditing({ ...editing, code: event.target.value })} /></td><td><input aria-label="코드명 수정" value={editing.name} onChange={(event) => setEditing({ ...editing, name: event.target.value })} /></td><td><input aria-label="코드 분류 수정" value={editing.category} onChange={(event) => setEditing({ ...editing, category: event.target.value })} /></td><td><RuleStateChoices active={editing.active} label={`${editing.name} 상태`} onChange={(active) => setEditing({ ...editing, active })} /></td><td><form className="edit-actions" onSubmit={saveEdit}><button className="save" type="submit">저장</button><button type="button" onClick={() => setEditing(null)}>취소</button></form></td></tr> : <tr key={code.id}><td>{code.id}</td><td><code>{code.code}</code></td><td><b>{code.name}</b></td><td>{code.category}</td><td><RuleStateChoices active={code.active} label={`${code.name} 상태`} onChange={(active) => { setCodes(codes.map((item) => item.id === code.id ? { ...item, active } : item)); showNotice(`${code.name} 코드를 ${active ? "활성" : "비활성"}화했습니다.`); }} /></td><td><button type="button" aria-label={`${code.name} 코드 수정`} onClick={() => setEditing({ ...code })}>✎ 수정</button></td></tr>)}</tbody></table></ScrollTable>
  </article>;
}

function MasterNote({ codeMode }: { codeMode: boolean }) {
  return <article className="card master-note"><h3>{codeMode ? "코드 적용 안내" : "규칙 적용 안내"}</h3><p>{codeMode ? "코드 변경은 연계 시스템 검토 후 배포됩니다." : "변경된 규칙은 승인 후 다음 수집 주기부터 적용됩니다."}</p><div className="step current"><b>1</b><span>변경사항 저장</span></div><div className="step"><b>2</b><span>검토자 승인</span></div><div className="step"><b>3</b><span>운영 환경 반영</span></div></article>;
}

function RuleManagement({ title, idLabel, scopeLabel, thresholdLabel, idPrefix, filename, rules, setRules, showNotice }: { title: string; idLabel: string; scopeLabel: string; thresholdLabel: string; idPrefix: string; filename: string; rules: Rule[]; setRules: (rules: Rule[]) => void; showNotice: (message: string) => void }) {
  const [draft, setDraft] = useState({ name: "", scope: "", threshold: "" });
  const [editing, setEditing] = useState<Rule | null>(null);
  const updateDraft = (field: keyof typeof draft, value: string) => setDraft({ ...draft, [field]: value });
  const addRule = (event: React.FormEvent) => {
    event.preventDefault();
    if (!draft.name.trim() || !draft.scope.trim() || !draft.threshold.trim()) return showNotice("규칙명, 적용 범위, 임계값을 모두 입력해 주세요.");
    const nextNumber = Math.max(0, ...rules.map((rule) => Number(rule.id.split("-")[1]) || 0)) + 1;
    setRules([...rules, { id: `${idPrefix}-${String(nextNumber).padStart(3, "0")}`, name: draft.name.trim(), scope: draft.scope.trim(), threshold: draft.threshold.trim(), active: true }]);
    setDraft({ name: "", scope: "", threshold: "" });
    showNotice(`${title}에 새 규칙을 추가했습니다.`);
  };
  const saveEdit = (event: React.FormEvent) => {
    event.preventDefault();
    if (!editing || !editing.name.trim() || !editing.scope.trim() || !editing.threshold.trim()) return showNotice("규칙명, 적용 범위, 임계값을 모두 입력해 주세요.");
    setRules(rules.map((rule) => rule.id === editing.id ? { ...editing, name: editing.name.trim(), scope: editing.scope.trim(), threshold: editing.threshold.trim() } : rule));
    setEditing(null);
    showNotice("규칙 변경사항을 저장했습니다.");
  };
  const setActive = (rule: Rule, active: boolean) => {
    setRules(rules.map((item) => item.id === rule.id ? { ...item, active } : item));
    showNotice(`${rule.name} 규칙을 ${active ? "활성" : "비활성"}화했습니다.`);
  };
  return <article className="card rules master-surface">
    <div className="section-head"><div><h3>{title}</h3><p>{rules.length}개 규칙</p></div><button type="button" onClick={() => { downloadMasterCsv([idLabel, "규칙명", scopeLabel, thresholdLabel, "상태"], rules.map((rule) => [rule.id, rule.name, rule.scope, rule.threshold, rule.active ? "활성" : "비활성"]), filename); showNotice(`${filename} 파일을 내려받았습니다.`); }}>⇩ 내보내기</button></div>
    <form className="master-create rule-create" onSubmit={addRule}>
      <label>규칙명<input aria-label="새 규칙명" value={draft.name} onChange={(event) => updateDraft("name", event.target.value)} placeholder="규칙명 입력" /></label>
      <label>{scopeLabel}<input aria-label={`새 ${scopeLabel}`} value={draft.scope} onChange={(event) => updateDraft("scope", event.target.value)} placeholder="적용 범위 입력" /></label>
      <label>{thresholdLabel}<input aria-label={`새 ${thresholdLabel}`} value={draft.threshold} onChange={(event) => updateDraft("threshold", event.target.value)} placeholder="임계값 입력" /></label>
      <button className="black" type="submit">＋ 새 규칙 추가</button>
    </form>
    <ScrollTable label={`${title} 목록`}><table><thead><tr><th>{idLabel}</th><th>규칙명</th><th>{scopeLabel}</th><th>{thresholdLabel}</th><th>상태</th><th>작업</th></tr></thead><tbody>{rules.map((rule) => editing?.id === rule.id ? <tr className="master-edit-row" key={rule.id}><td>{rule.id}</td><td><input aria-label="규칙명 수정" value={editing.name} onChange={(event) => setEditing({ ...editing, name: event.target.value })} /></td><td><input aria-label="적용 범위 수정" value={editing.scope} onChange={(event) => setEditing({ ...editing, scope: event.target.value })} /></td><td><input aria-label="임계값 수정" value={editing.threshold} onChange={(event) => setEditing({ ...editing, threshold: event.target.value })} /></td><td><RuleStateChoices active={editing.active} label={`${editing.name} 상태`} onChange={(active) => setEditing({ ...editing, active })} /></td><td><form className="edit-actions" onSubmit={saveEdit}><button className="save" type="submit">저장</button><button type="button" onClick={() => setEditing(null)}>취소</button></form></td></tr> : <tr key={rule.id}><td>{rule.id}</td><td><b>{rule.name}</b></td><td>{rule.scope}</td><td>{rule.threshold}</td><td><RuleStateChoices active={rule.active} label={`${rule.name} 상태`} onChange={(active) => setActive(rule, active)} /></td><td><button type="button" aria-label={`${rule.name} 규칙 수정`} onClick={() => setEditing({ ...rule })}>✎ 수정</button></td></tr>)}</tbody></table></ScrollTable>
  </article>;
}
