const PERIOD_DAYS = { all: null, "7d": 7, "30d": 30, quarter: 90 };
...
export function buildDashboardResponse({ filters, now, alarms, targets, actionPlans }) {
  ...
  const filteredAlarms = alarms.filter((alarm) => (
    (!since || new Date(alarm.occurredAt) >= since)
    && (filters.factory === "all" || alarm.customer === filters.factory)
    && (filters.productType === "all" || alarm.item === filters.productType)
  ));
  const alarmIds = new Set(filteredAlarms.map((alarm) => alarm.id));
  const includeStandaloneTargets = filters.period === "all" && filters.factory === "all" && filters.productType === "all";
  const filteredTargets = targets.filter((target) => !target.sourceAlarmId
    ? includeStandaloneTargets
    : alarmIds.has(target.sourceAlarmId));
  ...
  return {
    kpi: {
      ...
      criticalAlerts: filteredAlarms.filter((alarm) => alarm.status === "심각").length,
      newAlerts: filteredAlarms.filter((alarm) => alarm.status === "신규").length,
      inProgressTargets: filteredTargets.filter((target) => target.status === "진행 중").length,
      overdueTargets: overdueCases.length,
    },
    ...
    chartData: {
      alertTrend: [...trend.entries()].sort(([a], [b]) => a.localeCompare(b)).map(([date, count]) => ({ date, count })),
      targetDistribution: [...distribution.entries()].map(([status, count]) => ({ status, count })),
    },
    filterOptions: {
      customers: [...new Set(alarms.map((alarm) => alarm.customer).filter(Boolean))].sort((a, b) => a.localeCompare(b, "ko")),
      products: [...new Set(alarms.map((alarm) => alarm.item).filter(Boolean))].sort((a, b) => a.localeCompare(b, "ko")),
    },
  };
}
