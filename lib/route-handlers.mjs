...
function parseActionPlan(input) {
  ...
  if (input.tasks !== undefined && (!Array.isArray(input.tasks) || input.tasks.some((task) => {
    ...
    return !requiredText(task.description) || !requiredText(task.owner) || optionalDate(task.dueDate) === undefined || optionalDate(task.completedAt) === undefined;
  }))) return null;
  return {
    status, alarmId, targetId, targetStatus, ...values,
    tasks: (input.tasks ?? []).map((task) => ({
      description: requiredText(task.description),
      owner: requiredText(task.owner),
      dueDate: optionalDate(task.dueDate) ?? null,
      completedAt: optionalDate(task.completedAt) ?? null,
    })),
  };
}

function parseClosureActionPlan(input) {
  const targetId = requiredText(input.targetId);
  const rootCause = requiredText(input.rootCause);
  const immediateAction = requiredText(input.immediateAction);
  const preventiveAction = requiredText(input.preventiveAction);
  const closureReason = requiredText(input.closureReason);
  if (!targetId || !isUuid(targetId) || !rootCause || !immediateAction || !preventiveAction || !closureReason || !Array.isArray(input.tasks) || !input.tasks.length) return null;
  const tasks = input.tasks.map((task) => {
    if (!task || typeof task !== "object") return null;
    const description = requiredText(task.description);
    const owner = requiredText(task.owner);
    const dueDate = optionalDate(task.dueDate);
    const completedAt = optionalDate(task.completedAt);
    if (!description || !owner || !dueDate || !completedAt) return null;
    return { description, owner, dueDate, completedAt };
  });
  if (tasks.some((task) => !task)) return null;
  return { targetId, rootCause, immediateAction, preventiveAction, closureReason, tasks };
}
...
export function createAlarmDetailRouteHandlers(repository) {
  return {
    async GET(_request, context) {
      ...
      try {
        const detail = await repository.getAlarmDetail(id);
        if (!detail) return json({ error: "알람을 찾을 수 없습니다." }, { status: 404 });
        return json(detail);
...
export function createTargetRouteHandlers(repository) {
  return {
    ...
    async POST(request) {
      ...
      try {
        const sourceAlarm = target.sourceAlarmId ? await repository.findAlarm(target.sourceAlarmId) : null;
        if (target.sourceAlarmId && !sourceAlarm) {
          return json({ error: "연결할 알람을 찾을 수 없습니다." }, { status: 404 });
        }
        if (target.sourceAlarmId) {
          const existingTarget = await repository.findTargetBySourceAlarm(target.sourceAlarmId);
          if (existingTarget) {
            return json({ error: "이미 관리대상으로 등록된 알람입니다.", target: existingTarget }, { status: 409 });
          }
        }
        const sourceItem = typeof sourceAlarm?.item === "string" ? sourceAlarm.item.trim() : "";
        const targetToCreate = sourceItem ? { ...target, name: sourceItem } : target;
        const created = await repository.createTargetWithAudit(targetToCreate, { eventType: "target.created", entityType: "target", details: { name: targetToCreate.name } });
...
export function createActionPlanCloseRouteHandlers(repository) {
  return {
    async POST(request, context) {
      const body = await readJson(request);
      const closure = body && parseClosureActionPlan(body);
      if (!closure) return json({ error: "조치계획 종결 입력값이 올바르지 않습니다." }, { status: 400 });
      try {
        const { id } = await context.params;
        if (!isUuid(id)) return json({ error: "조치계획 식별자가 올바르지 않습니다." }, { status: 400 });
        if (!await repository.findTarget(closure.targetId)) return json({ error: "관리대상을 찾을 수 없습니다." }, { status: 404 });
        const actionPlan = await repository.closeActionPlanWithAudit(id, closure.targetId, {
          ...closure,
          status: "종결",
          targetStatus: "완료",
        }, {
          eventType: "action-plan.closed",
          entityType: "action-plan",
          details: { targetId: closure.targetId, status: "종결", targetStatus: "완료" },
        });
        if (!actionPlan) return json({ error: "조치계획을 찾을 수 없습니다." }, { status: 404 });
        return json({ actionPlan });
      } catch {
        return json({ error: "조치계획을 종결할 수 없습니다." }, { status: 500 });
      }
    },
  };
}
