...
export function createActionPlanRouteHandlers(repository) {
  return {
    async POST(request) {
      ...
      try {
        const created = await repository.createActionPlanWithAudit(actionPlan, { eventType: "action-plan.created", entityType: "action-plan", details: { status: actionPlan.status } });
